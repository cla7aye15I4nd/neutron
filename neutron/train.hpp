...
class train {
	...
	long long combine(int userID, int date = 0, int trainID = 0, int start = 0, int end = 0) {
...
public:
	...
	bool sale() {
		...
		t.update(h);
		ticketMgr.addNewTrain(h, t.numStation);
		return true;
	}
	void qry(bool sudo = 0) {
		trainID.read();
	...
	void qryDirect(bool mhb = false, bool brief = false) {
		loc1.read();
		loc2.read();
		date_s.read();
		catalog.read();
		...
		for (int i = 0; i < cnt; i++) {
			...
			t.print_detailed(start, end, date, mhb, brief);
		}
	}
	void qryTrans(bool mhb = false, bool brief = false) {
		loc1.read();
		loc2.read();
		date_s.read();
		catalog.read();
		...
		t.print_detailed(start, mtra, date, mhb, brief);
		hashT.getTrainData(mt2ID, t);
		t.print_detailed(mtra, end, date, mhb, brief);
	}
	bool buy() {
	...
	void qryTicket(bool sudo = false) {
		scanf("%d", &userID);
		date_s.read();
		catalog.read();
		date = (date_s[8] - '0') * 10 + (date_s[9] - '0');
